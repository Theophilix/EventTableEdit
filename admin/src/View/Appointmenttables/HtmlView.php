...
namespace ETE\Component\EventTableEdit\Administrator\View\Appointmenttables;
...
class HtmlView extends BaseHtmlView {
	...
	protected function addToolbar()
	{
		...
		ToolbarHelper::title(Text::_('COM_EVENTTABLEEDIT_SUBMENU_APPOINTMENTTABLES'), '');

		if ($canDo->get('core.create') || \count($user->getAuthorisedCategories('com_eventtableedit', 'core.create')) > 0)
		{
			$toolbar->addNew('appointmenttable.add');
		}
		...
		if (!$this->isEmptyState && $canDo->get('core.edit'))
		{
			...
			$childBar->publish('appointmenttables.publish')->listCheck(true);

			$childBar->unpublish('appointmenttables.unpublish')->listCheck(true);

			$childBar->archive('appointmenttables.archive')->listCheck(true);

			if ($user->authorise('core.admin'))
			{
				$childBar->checkin('appointmenttables.checkin')->listCheck(true);
			}

			if ($this->state->get('filter.published') != -2)
			{
				$childBar->trash('appointmenttables.trash')->listCheck(true);
			}
		...
		{
			$toolbar->delete('appointmenttables.delete')
